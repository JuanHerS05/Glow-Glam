...
export default function EditProductForm({ barcodeParam }) {
  const [product, setProduct] = useState({ idBarcode: '', name: '', category: '', brand: '', description: '', price: 0, active: true });
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        if (barcode) {
          const respProd = await fetch(`/api/products/barcode/${encodeURIComponent(barcode)}`, { credentials: 'include' });
          if (respProd.ok) {
            const prodData = await respProd.json();
            let categoryName = '';
            if (prodData.category) {
              categoryName = typeof prodData.category === 'object' ? (prodData.category.name || '') : prodData.category;
            }
            setProduct({
  ...
  const toggleActiveStatus = () => setProduct((prev) => ({ ...prev, active: !prev.active }));
  ...
  const removeUrlField = (indexToRemove) => setImagesUrls(imagesUrls.filter((_, index) => index !== indexToRemove));
    ...
    <div className="bodyWrapper" style={{ position: 'relative', minHeight: '100vh' }}>
      ...
      <main className="contenedorFormulario" style={{ position: 'relative', zIndex: 1 }}>
        <div className="panelEdicion">
          <h3>Editando: <span className="productNameHighlight">{product.name || 'Nombre Producto'}</span></h3>
          <form onSubmit={handleSubmit}>
            ...
            <div className="estadoControl">
              <span>Estado actual: <strong className={product.active ? 'txtActivo' : 'txtInactivo'}>{product.active ? 'Activo' : 'Inactivo'}</strong></span>
              <button type="button" className="btnEstado" onClick={toggleActiveStatus}>Cambiar Estado</button>
            </div>
            <label>URLs de las imágenes del Producto:</label>
            <div className="urlInputsContainer">
              {imagesUrls.map((url, index) => (
                <div key={index} className="urlInputGroup">
                  <input type="url" name="imageUrlInput" value={url} placeholder="https://ejemplo.com/imagen.jpg" onChange={(e) => handleUrlInputChange(index, e.target.value)} required={index === 0} />
                  {index === 0 ? (
            ...
            <div className="imagenesHorizontalPreview">
              {imagesUrls.map((url, idx) => {
                ...
                return (<img key={idx} src={url.trim()} alt={`Preview ${idx + 1}`} onError={(e) => { e.target.style.display = 'none'; }} className="previewImageItem" />);
              })}
            </div>
      ...
      {showSuccessModal && (
        <div className="modalConfirmacionContainer" style={{ zIndex: 100000 }}>
          <div className="modalConfirmacionContenido">
            ...
            <br />
            <button type="button" onClick={() => setShowSuccessModal(false)} className="btnVolverInicio btnSeguirModificando">Seguir modificando productos</button>
          </div>
        </div>
      )}
    </div>
